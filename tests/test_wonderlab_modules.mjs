...
test('the catalog has the agreed twenty destinations and category balance', () => {
  ...
  assert.deepEqual(
    ...
    { games: 7, tools: 7, tabletop: 5, work: 1 }
  );
...
test('captured previews have complete, non-empty 640px and 1280px WebP variants', () => {
  ...
  assert.deepEqual(appsWithoutPreview, ['mission-control', 'traceglass', 'idea-graph', 'sudbury-regreening', 'permit-pulse', 'ypsilon-overkill', 'foundry', 'eems']);
...
test('public routes preserve external URLs, root-relative routes, and local-only state', () => {
  ...
  assert.equal(resolveAppUrl(appById('permit-pulse'), publicLocation), 'https://permitpulse.worldwidesam.net/');
...
test('local routes use the current machine hostname and each destination port', () => {
  ...
  assert.equal(resolveAppUrl(appById('permit-pulse'), lanLocation), 'https://permitpulse.worldwidesam.net/');
  ...
  assert.equal(resolveAppUrl(appById('permit-pulse'), loopbackLocation), 'https://permitpulse.worldwidesam.net/');
...
test('selection state remembers a category selection and wraps arrow movement', () => {
  ...
  assert.equal(selection.selectApp('idea-graph').id, 'idea-graph');
  assert.equal(selection.selectApp('sudbury-regreening').id, 'sudbury-regreening');
  assert.equal(selection.selectApp('permit-pulse').id, 'permit-pulse');
  assert.equal(selection.selectCategory('games').id, 'clawdtris');
  assert.equal(selection.selectCategory('tools').id, 'permit-pulse');
